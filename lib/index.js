...
function pushToSocketServer (params, callback) {
  ...
  pushToSNSTopic(params, (err) => {
    if (err) { AwsHelper.log.error({ ...params, error: err }, `Failed to publish SNS event to ${process.env.SEARCH_RESULT_TOPIC}`); }
  });
...
function saveRecordToS3 (params, callback) {
  ...
  params.items.forEach(function (item) {
    // params.userId is the clientId or "fingerprint"
