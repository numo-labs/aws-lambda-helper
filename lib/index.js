 ...
 * @param {String} clientId - the Primus connection id of the Client/Device
 * @param {String} searchId - the unique ID for the search perfomed by client
 *  for when client performs multiple searches but no longer wants "old" results
 * @param {Object} json - the result object that we want to send to client
 * @param {Function} callback - called when we get a reply from Primus Server
 */
function pushToSocketServer (clientId, searchId, json, callback) {
  var options = {
    headers: {
    ...
    'path': '/data',
    'method': 'POST',
    body: { // see: https://github.com/numo-labs/aws-lambda-helper/issues/40
      id: clientId,
      searchId: searchId,
 ...
 * @param 
 */
// process.env.AWS_S3_BUCKET = 'numo-search-results';
function saveRecordToS3 (sessionId, json, callback) {
  var filepath = sessionId + '/' + json.id + '.json';
  ...
  return resultBucket.upload(params, callback);
}
AwsHelper.saveRecordToS3 = saveRecordToS3; // export for testing
//
// AwsHelper.pushResult = function(bucketId, json, callback) {
