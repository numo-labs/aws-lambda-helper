...
function httpRequest (options, callback) {
  ...
  var req = proto(options).request(options, function (res) {
    ...
    res.on('data', function (chunk) {
      // console.log('>', chunk); // uncomment to debug
      // console.log(' - - - - - - - - - - - - - - - - - - - - - - - - - - -');
      resStr += chunk;
    }).on('end', function () {
      if (resStr.length > 0) {
...
//
// function saveRecordToS3 (bucketId, recordId, json, callback) {
//
// }
// AwsHelper.saveRecordToS3 = saveRecordToS3; // export it for testing
//
