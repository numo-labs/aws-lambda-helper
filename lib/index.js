...
function httpRequest (options, callback) {
  ...
  var req = proto(options).request(options, function (res) {
    ...
    res.on('data', function (chunk) {
      ...
      if (resStr.length > 0 && res.statusCode === 200) {
        try {
        ...
        if (res.statusCode === 200) {
          ...
          return callback(res.statusCode, resStr);
        }
      }
    });
  });
...
function pushToSocketServer (params, callback) {
  ...
  //   body: params
  // };
  // trigger SNS event with search result - for dev right now, so ignore errors
  pushToSNSTopic(params, (err) => { // @lennym to update this ...
    /* istanbul ignore else */
    if (err) {
      AwsHelper.log.error({ params: params, error: err },
      `Failed to publish SNS event to ${process.env.SEARCH_RESULT_TOPIC}`);
    }
  });
...
/**
 * pushResultToClient wraps both pushToSocketServer and saveRecordToS3,
 * so that your lambda code does not have to call both methods.
 * the params are the same. (see README for example)
 * @param {Object} params - the parameters you want to send to client/S3
 * @param {Function} callback - called when the method succeeds/fails
 */
